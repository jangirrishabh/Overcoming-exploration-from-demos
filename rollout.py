...
class RolloutWorker:
    ...
    def __init__(self, madeEnv, make_env, policy, dims, logger, T, rollout_batch_size=1,
        ...
        """
        #self.envs = [make_env() for _ in range(rollout_batch_size)] #comment out when a way to use multiple gazebo envs simulataneously is found, or with normal mujoco envs
        self.envs = [madeEnv] # comment out when using Gazebo environments
        assert self.T > 0
...
class RolloutWorkerOriginal:
    ...
    def reset_rollout_from_demo(self, i):
        """Resets the `i`-th rollout environment, re-samples a new goal, and updates the `initial_o`
        and `g` arrays accordingly.
        """
        

        [reset_state, reset_goal] = self.policy.sample_demo_state()

        obs = self.envs[i].reset_mutated(reset_state, reset_goal)
        self.initial_o[i] = obs['observation']
        self.initial_ag[i] = obs['achieved_goal']
        self.g[i] = obs['desired_goal']
    ...
    def reset_all_rollouts_from_demo(self):
        """Resets all `rollout_batch_size` rollout workers.
        """
        for i in range(self.rollout_batch_size):
            self.reset_rollout_from_demo(i)


    def generate_rollouts_from_demo(self):
        """Performs `rollout_batch_size` rollouts in parallel for time horizon `T` with the current
        policy acting on it accordingly.
        """
        self.reset_all_rollouts_from_demo()
...
class RolloutWorkerSofa:

    @store_args
    def __init__(self, service_name, policy, dims, logger, T, rollout_batch_size=1,
                 exploit=False, use_target_net=False, compute_Q=False, noise_eps=0,
                 random_eps=0, history_len=100, render=False, **kwargs):
        """Rollout worker generates experience by interacting with one or many environments.
    ...
    #             self.logger.warn('NaN caught during rollout generation. Trying again...')
    #             self.reset_all_rollouts_from_demo()
    #             return self.generate_rollouts_from_demo()
        ...
        for t in range(self.T):
            ...
            for i in range(self.rollout_batch_size):
                try:
                    ...
                    del curr_o_new['done']
                    info = { 'is_success' : response['is_success']}
    ...
    # def seed(self, seed): # seeding of the sofa env to be figured out later
    #     """Seeds each environment with a distinct seed derived from the passed in global seed.
    #     """
    #     for idx, env in enumerate(self.envs):
    #         env.seed(seed + 1000 * idx)
